...
<header class="c-header c-header-light c-header-fixed c-header-with-subheader">
    ...
    $activeStatus = DB::table('professional_active_status')
        ->where(['professional_id' => auth()->user()->id])->first() ?? 0;
    ?>

    <ul class="c-header-nav ml-auto mr-2">
        <li class="mx-2">
            <div class="custom-control custom-switch">
                <input type="checkbox" class="custom-control-input" id="active-status" data-onstyle="success" 
                data-offstyle="danger" data-toggle="toggle" data-on="Active" data-off="Inactive" 
                {{ $activeStatus->status == 1 ? 'checked' : '' }}>
